...
const artifactDomain = "dsh-sales";
...
function reviewArtifact(value, expectedType) {
	...
	const stale$1 = validDate(staleAfter) && Date.parse(staleAfter) <= Date.now();
	if (stale$1) warnings.push(`工件已过期：${staleAfter}。`);
	const status = issues.length > 0 ? "blocked" : stale$1 ? "stale" : warnings.length > 0 ? "partial" : "ready";
	return {
		status,
		issues,
		warnings,
...
async function appendArtifactAudit(fs, root, event, signal) {
	const path = join(root, ".dsh-sales-audit.jsonl");
...
function receiveHandoff(value, options) {
	...
	const missing = [];
	const sourceType = text$1(source.artifactType);
	...
	if (route) {
		...
		for (const field$1 of route.text) if (!text$1(source[field$1])) missing.push(field$1);
		for (const field$1 of route.lists) {
			const values = source[field$1];
			if (!Array.isArray(values) || values.length === 0 || !values.every(meaningful)) missing.push(field$1);
		}
	}
	for (const field$1 of [
		"initiativeId",
		"owner",
		"action"
	]) if (!options[field$1].trim()) missing.push(field$1);
	...
	if (sourceType === "product-sales-handoff" && !["proceed", "scale"].includes(text$1(source.productDecision))) missing.push("productDecision");
	...
	const nextActions = status === "accepted" ? [
		...
		...issues,
		...missing.map((field$1) => `补齐或核验：${field$1}。`),
...
function parseDelimited(text$2, delimiter) {
	const rows = [];
	let row = [];
	let cell = "";
	let quoted = false;
	for (let index = 0; index < text$2.length; index += 1) {
		const char = text$2[index];
		const next = text$2[index + 1];
...
async function readSalesDataset(fs, config, path, signal) {
	...
	const text$2 = await fs.readText(target, signal);
	if (text$2.length > config.maxTextChars) throw new Error(`File exceeds maxTextChars (${config.maxTextChars})`);
	const ext = extension$1(path);
	let rows;
	const warnings = [];
	if (ext === ".csv" || ext === ".tsv") {
		const table = parseDelimited(text$2, ext === ".tsv" ? "	" : ",");
	...
	} else if (ext === ".jsonl" || ext === ".ndjson") rows = text$2.split(/\r?\n/).filter((line) => line.trim()).slice(0, config.maxRows).map((line, index) => normalizeRow(JSON.parse(line), index));
	else if (ext === ".json") rows = objectRows(JSON.parse(text$2)).slice(0, config.maxRows).map(normalizeRow);
...
function renderResult(value, maxChars) {
	...
	return [{
		type: "text",
		text: text$2.length > maxChars ? `${text$2.slice(0, maxChars)}\n... result truncated by dsh-sales; use a narrower source or scope ...` : text$2
	}];
}
...
function normalizeCrmExport(dataset, mapping = {}) {
	...
	const records = dataset.rows.map((row) => {
		const normalized$1 = { ...row };
		for (const [key, source] of Object.entries(fieldMap)) normalized$1[key] = row[source];
		return normalized$1;
	});
	const warnings = [...dataset.warnings];
	for (const key of [
...
function wrapResult(value, options = {}) {
	...
	return resultEnvelope({
		data: jsonValue(attachArtifactMetadata(value, { staleAfterDays: 30 })),
		warnings,
		assumptions: options.assumptions,
		lineage: options.lineage,
		nextActions: options.nextActions
...
function registerSalesTools(ctx, config, fs) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			const data = typeof parsed.data === "object" && parsed.data !== null && !Array.isArray(parsed.data) ? parsed.data : parsed;
			const integrity = reviewArtifact(data, "commercial-handoff");
			if (integrity.status !== "ready" || typeof parsed === "object" && parsed !== null && "data" in parsed && (!("ok" in parsed) || parsed.ok !== true)) return wrapResult({
				artifactType: "commercial-handoff-review",
				status: "blocked",
				issues: integrity.issues,
				warnings: integrity.warnings,
				nextActions: ["先重新生成有效来源，再调用 handoff_receive；不能跳过完整性与有效期检查。"]
			});
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			return wrapResult({
				artifactType: "sales-artifact-review",
				generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
				...review
			}, { nextActions: review.nextActions });
		}
